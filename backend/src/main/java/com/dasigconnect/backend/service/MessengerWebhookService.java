...
public class MessengerWebhookService {
    ...
    public MessengerWebhookService(
            @Value("${app.messenger.verify-token:}") String verifyToken,
            @Value("${app.messenger.app-secret:}") String appSecret,
    ...
    public boolean isSignatureValid(byte[] payload, String signatureHeader) {
        if (appSecret.isBlank() || signatureHeader == null
    ...
    /**
     * Event-specific linking and message handling is added behind this boundary.
     * Meta requires a prompt 200 response, so never log the payload or do remote work here.
     */
                ...
                for (JsonNode event : entry.path("messaging")) {
                    ...
                    connections.link(code, psid).ifPresentOrElse(
                            userId -> delivery.sendToPsid(psid,
                                    "Messenger notifications are now connected to your DASIGConnect validator account."),
                            () -> delivery.sendToPsid(psid,
                                    "That connection code is invalid or expired. Generate a new code in DASIGConnect."));
                }
            }
        } catch (Exception ex) {
