...
- **A2 — Withdraw Submission:** was completely broken, now fixed (2026-09-13). `SubmissionService.withdraw()` itself was correct and fully unit-tested, but no controller endpoint ever called it — `POST /api/v1/submissions/{id}/withdraw` (the exact route the frontend's `withdrawSubmission()` already called) simply did not exist on `SubmissionController`, so every withdraw attempt 404'd regardless of status or review-lock state. Added the missing `@PostMapping("/{id}/withdraw")` (`hasAnyRole('CONTRIBUTOR','MODERATOR','ADMIN')`, matching `submit()`'s pattern) wired to the existing service method. With that in place, the underlying logic is as previously documented and remains accurate: withdrawal is blocked not just once a Moderator/Admin "begins reviewing" informally, but the instant a `ReviewLock` row exists for the submission (opening it in the queue and acquiring the lock via **Start Review** creates it — merely opening/viewing does not) — `409 Conflict`, "This submission is already under review and can no longer be withdrawn." Withdrawing is also only valid from `pending` specifically (not `in_review`) — returns the draft to `draft` and clears `submittedAt`.
...
- **The post's album only applies to fresh uploads.** On submit, an album-less (freshly uploaded) asset is filed into the album named on the post, but a library-picked asset keeps the album it already had — deliberately, so reusing an image in a new post never moves it. So the post's album and where a given image actually lives can differ. Since 2026-09-23 the read-only submission view (`SubmissionReadOnlyView`) shows each image's own album under its file name whenever it differs from the post's, plus a one-line note on the Album row explaining why; before that, the only visible album was the post's, which read as if every image had been filed there.
