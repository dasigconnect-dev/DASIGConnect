...
**Category removed from the AI caption context (2026-09-12):** the Main Flow previously listed "category" among what's sent to Claude. This was stale: the composer's `toPayload()` (`frontend/src/features/submission/utils.ts`) has always hardcoded `category: ""` when creating/updating a submission — there is no UI field that lets a contributor set it, so `Submission.category` is permanently `null` for every submission the composer creates, and `appendContextLine` silently dropped the blank value from the prompt anyway. Removed the dead parameter end-to-end: `ClaudeVisionClient.generateCaptions`/`buildPayload`/`buildPrompt`/`buildSubmissionContext` and `CaptionGenerationService.SubmissionContext` no longer carry a `category` field. (The `Submission.category` DB column and DTOs are untouched — they're still read/written elsewhere, e.g. built-in post templates and UC-3.3 media-suggestion re-ranking; only the caption-generation pathway is affected.)
