...
| Deletion lifecycle / Trash & recovery | ✅ | UC-2.2 — soft-delete (role-scoped: contributor=own, validator=institution, admin=all) → 30-day retention → automatic purge (job). **Trash UI added (2026-06-07):** `GET /media-assets/trash` + `POST /{id}/restore` + `POST /{id}/purge` (`MediaTrashService`). Any user can **Restore** or **Delete forever** (immediate purge) items in their trash; admins get an All + per-institution scope view. Restore is lossless — embeddings are now retained through the trash window and removed only at purge. Purge keeps the audit tombstone + integrity history. |
| Bulk deletion safety / used-media handling | ✅ | **Implemented 2026-06-07.** Multi-select pages expose **Select all** after the first selected item. Media Library and Collection bulk delete run as a sequential queue with progress counts. If a referenced asset is encountered, normal delete returns 409 and the UI pauses for **Skip and continue** or **Force delete and continue**. Force retries with `force=true`, is audited, and warns that draft/pending/active submissions may need replacement media. Trash delete-forever remains permanent purge and bypasses skip/force because the asset has already left the active library. |
...
| Quality / duplicate management | 🟡 | UC-4.4 — Phase 2 foundation adds `perceptual_hash`, `blur_score`, `duplicate_of_id`; deterministic computation runs in the ingestion queue. **Duplicate review UI shipped (Phase 7F).** **Upload-time duplicate handling (2026-06-07):** the upload flow detects a **same-filename** asset in the current location (client-side, via `useUploadOptions` + `findExistingByName` over the loaded library, scoped to the current folder) and shows a Google Drive-style **Upload options** modal — **Replace existing file** (uploads as a `replacement_for` linked new version and soft-deletes the superseded asset; originals stay immutable/recoverable) or **Keep both files**. A byte-level `POST /media-assets/check-duplicates` (SHA-256) endpoint also exists as a complementary signal (depends on the Phase 7A `content_sha256` baseline being populated). Near-duplicate (different bytes, similar image) still routes through post-upload `perceptual_hash` flagging + the 7F review workspace. D1 threshold tuning / human-labeled set still pending. **Browse fix:** the Media Library now loads the full library (was capped at the first 25 by an unpaginated fetch). |
...
| Metadata-standard mapping (Dublin Core/PREMIS) | ✅ | **Phase 7G implemented (2026-06-06).** `GET /api/v1/media-repository/export?format=csv\|json` (`MediaRepositoryExportService`, audited `MEDIA_REPOSITORY_EXPORTED`) emits a tenant-scoped, Dublin Core-aligned inventory (mapping in `docs/md/dublin-core-mapping.md`) with `pres_*` fixity/provenance/lifecycle fields. Omits `storage_url` and all signed/private URLs; PREMIS concepts limited honestly to fixity + preservation-event history. JSON embeds the DC mapping for portability. |
