...
| Quality / duplicate management | 🟡 | UC-4.4 — Phase 2 foundation adds `perceptual_hash`, `blur_score`, `duplicate_of_id`; deterministic computation runs in the ingestion queue. **Duplicate review UI shipped (Phase 7F).** **Upload-time exact-duplicate check added (2026-06-06):** the browser computes each file's SHA-256 and `POST /media-assets/check-duplicates` flags files already in the institution's library; the upload modal skips exact re-uploads by default with an "Upload anyway" override (Drive-style). Near-duplicate (different bytes, similar image) still routes through post-upload `perceptual_hash` flagging + the 7F review workspace. D1 threshold tuning / human-labeled set still pending. **Browse fix:** the Media Library now loads the full library (was capped at the first 25 by an unpaginated fetch). |
