...
6. The actor writes the **caption** directly or via **Suggest Caption** (AI, UC-1.6) with an optional tone/instruction prompt; the AI can reference the attached media and returns **one** caption in the selected tone to insert, edit, or discard. The caption has a **hard character cap** (`CAPTION_CHAR_LIMIT` = 3000 code points; backend `MAX_CAPTION_CHARS`) — pasting past it trims with a warning, and the backend rejects an over-length caption on create/update (`400`) and submit (`422`). Caption length within a comfortable range is *also* a non-blocking Recommended readiness check.
...
- **A10 — Generate Template from Top-Performing Posts** *(added 2026-09-23)*: From the template list, the actor chooses **Generate from Top Posts** (any role; not shown on read-only submissions). `POST /api/v1/ai/templates/from-top-posts` (`AiTemplateController` → `TopPostTemplateService`) returns an AI-drafted template; **nothing is saved server-side**.
