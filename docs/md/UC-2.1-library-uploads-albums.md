...
- **A5 — fixed (2026-09-13), UI included.** `MediaAssetService.updateAlbum` now accepts a null `albumId` as a deliberate "remove from all albums" request (`media_album_id` was always nullable — `STAGED` submission uploads already sit album-less pre-submit) rather than rejecting it. Moving to a different existing album already worked. `AssetDetailPanel`'s Album Organization section now has a **"Remove from album"** button (next to "Move here", shown whenever the asset currently has one) that calls `onUpdateAlbum(asset.id, null)`; `MediaRepositoryScreen.handleUpdateAssetAlbum` no longer silently no-ops a null `albumId` and drops the asset from the currently-viewed grid the same way a move-out-of-folder already did.
