...
- **A10 — Cross-Institution Asset Movement (Contributor: owner + shared-library only; Moderator, Administrator: unrestricted, any institution to any institution):** an asset may be moved into or out of the shared network-default institution's library (DASIG Central Visayas), in addition to reassignment within the actor's own institution (A5). Moderator is grouped with Administrator here, not Contributor — both are network-wide roles (`isNetworkRole` = Admin or Moderator) with identical unrestricted cross-institution move rights, and no "All institutions" view gate is required for either. Confirmed as the intended scoping — not a gap.
