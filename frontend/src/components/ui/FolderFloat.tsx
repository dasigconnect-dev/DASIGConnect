...
const layout = (
  ...
  rows.forEach((r, ri) => {
    let x = -r.width / 2;
    const shift = (ri % 2 ? 1 : -1) * Math.min(20, spread * 0.1);
...
export default function FolderFloat({
  ...
  const [sizes, setSizes] = useState<({ w: number; h: number } | null)[]>([]);
  ...
  const startPhysics = useCallback(() => {
    ...
    w.bodies = els.map((_el, i) => {
      ...
      (b as any).plugin = { phase: jitter(i) * Math.PI * 2 };
      return b;
    });
    const T = 80;
    const walls = [
    ...
    const tick = (now: number) => {
      ...
      const k = latest.current.drift * 0.00003 * Math.min(1, t / 2);
      s.bodies.forEach((b, i) => {
        if (s.drag && s.drag.i === i) return;
        const ph = (b as any).plugin.phase;
  ...
  const up = (e: PointerEvent<HTMLButtonElement>, i: number, item: FolderFloatItem) => {
    const w = world.current;
    const d = w.drag;
    if (!d || d.i !== i || d.id !== e.pointerId) return;
    w.drag = null;
          ...
          return (
            ...
            >
              <span className="folder-float__drift">
                {item.icon && <i className={item.icon} style={{ marginRight: 8, color: '#1877f2', fontSize: '1rem', verticalAlign: '-1px' }}></i>}
                {item.label}
              </span>
            </button>
          );
