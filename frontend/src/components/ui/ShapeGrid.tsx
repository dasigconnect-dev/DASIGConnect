...
export default function ShapeGrid({
  ...
  squareSize = 44,
  hoverFillColor = 'rgba(24, 119, 242, 0.12)',
  shape = 'square',
  hoverTrailAmount = 3,
  className = '',
}: ShapeGridProps) {
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      // Soft vignette gradient fading out edges smoothly on light canvas
      const maxDim = Math.sqrt(canvas.width ** 2 + canvas.height ** 2) / 2;
    ...
    const parent =
      (canvas.closest('section, .landing-cta-zone, .panel-r, .panel-l, .split, .screen') as HTMLElement) ||
  ...
  return <canvas ref={canvasRef} className={`shapegrid-canvas ${className}`} aria-hidden="true"></canvas>;
}
