...
export const GridScan: FC<GridScanProps> = ({
  enableWebcam = false,
  showPreview = false,
  sensitivity = 0.55,
  lineThickness = 1,
  ...
  const skewScale = enableMouseTracking ? THREE.MathUtils.lerp(0.06, 0.2, s) : 0;
  const tiltScale = enableMouseTracking ? THREE.MathUtils.lerp(0.12, 0.3, s) : 0;
  const yawScale = enableMouseTracking ? THREE.MathUtils.lerp(0.1, 0.28, s) : 0;
  ...
  useEffect(() => {
    const el =
      (containerRef.current?.closest('.landing-dark-zone') as HTMLElement | null) ||
  ...
  }, [uiFaceActive, snapBackDelay, scanOnClick, enableGyro, enableMouseTracking]);

  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;

    const getWidth = () => container.clientWidth || container.parentElement?.clientWidth || window.innerWidth || 300;
    const getHeight = () => container.clientHeight || container.parentElement?.clientHeight || window.innerHeight || 300;
